...
grn_main <- function(A=NULL, B ,seq=NULL, X=NULL, max.steps = 200, TF=NULL, gene, cl.cores=2, algorithm, HGSmat, file=TRUE, verbose=TRUE, bootstrap_num=5,bootstrap_seed=TRUE,sym = F,num = NULL){
  if (is.null(TF)) {
    TF <- gene
  }
  auclist <- c()
